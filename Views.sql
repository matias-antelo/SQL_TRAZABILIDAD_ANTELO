...
-- VIsta creada para ver trazabilidad de la esterilizacion del cultivo
CREATE OR REPLACE VIEW TRAZABILIDAD_PREPARACION_ESTERILIZACION AS
SELECT

    PMC.FECHA_PREPARACION,
    PMC.PH,
    PMC.OBSERVACIONES,
    AF1.NOMBRE AS ANALISTA_PREPARACION,
    
    EC.FECHA AS FECHA_CICLO,
    EC.TIEMPO,
    EC.TEMPERATURA,
    AF2.NOMBRE AS ANALISTA_ESTERILIZACION,
    
    EQ.MARCA,
    EQ.FECHA_CALIBRACION,
    EQ.VTO_CALIBRACION,
    
    AV.FECHA AS FECHA_VERIFICACION,
    AV.LECTURA_1,
    AV.LECTURA_2,
    AV.CUMPLE AS CUMPLE_AUTOCLAVE
           
FROM PREPARACION_MEDIO_CULTIVO PMC
LEFT JOIN ESTERILIZACION EC
    ON PMC.ID_CICLO = EC.ID_CICLO
LEFT JOIN EQUIPOS EQ
    ON EC.ID_EQUIPO = EQ.ID_EQUIPO
LEFT JOIN AUTOCLAVE_VERIF AV
    ON EQ.ID_EQUIPO = AV.ID_EQUIPO
LEFT JOIN ANALISTA_FIRMA AF1
    ON PMC.ID_ANALISTA = AF1.ID_ANALISTA
LEFT JOIN ANALISTA_FIRMA AF2
    ON EC.ID_ANALISTA = AF2.ID_ANALISTA;

-- VIsta creada para ver trazabilidad del medio para el control de calidad
CREATE OR REPLACE VIEW V_TRAZABILIDAD_MEDIOS AS
SELECT
    PMC.FECHA_PREPARACION,
    PMC.PH,
    PMC.OBSERVACIONES,
    AF2.NOMBRE AS ANALISTA_PREPARACION,
    
    MCS.NOMBRE AS MEDIO_NOMBRE,
    MCS.FABRICANTE,
    MCS.LOTE,
    MCS.VENCIMIENTO,
    
    EC.FECHA AS FECHA_ECOMETRICO,
    EC.ICA,
    EC.ICE,
    EC.RESULTADO AS RESULTADO_ECOMETRICO,
    
    AD.FECHA_ENTRADA AS FECHA_AGUA,
    AD.CONTROL AS CONTROL_AGUA,
    AF1.NOMBRE AS ANALISTA_AGUA
    
    
FROM PREPARACION_MEDIO_CULTIVO PMC
LEFT JOIN MEDIO_CULTIVO_STOCK MCS
    ON PMC.ID_MEDIO = MCS.ID_MEDIO
LEFT JOIN ECOMETRICO EC
    ON PMC.ID_ECOMETRICO = EC.ID_ECOMETRICO
LEFT JOIN AGUA_DEST AD
    ON PMC.ID_AGUA = AD.ID_AGUA
LEFT JOIN ANALISTA_FIRMA AF1
    ON AD.ID_ANALISTA = AF1.ID_ANALISTA
LEFT JOIN ANALISTA_FIRMA AF2
    ON PMC.ID_ANALISTA = AF2.ID_ANALISTA;